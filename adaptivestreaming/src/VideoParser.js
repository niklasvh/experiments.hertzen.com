...
function VideoParser(arraybuffer) {
...
VideoParser.prototype.parseInit = function() {
  var ab = this.buffer;
...
VideoParser.prototype.parseSIDX = function(ab_first_byte_offset) {
  var ab = this.buffer;
  var d = new DataView(ab);
  var pos = 0;
  ...
  for (var i = 0; i < reference_count; i++) {
    ...
    if (ref_type) {
      throw "Unhandled indirect reference";
    }
